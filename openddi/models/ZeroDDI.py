...
class ZeroDDI(nn.Module):
    ...
    def update_event_U(self, event_sem: torch.Tensor):
        ...
        """
        device = self._dev()
        assert event_sem is not None, "event_sem is empty"
        K, d_e = event_sem.shape
        assert K == self.num_classes, f"Number of events {K} must match num_classes={self.num_classes}"
    ...
    def forward(self, graph_or_none, idx_batch):
        ...
        if graph_or_none is not None:
            ...
            assert self.x_cache is not None and self.edge_index_cache is not None, \
                "Graph not bound and no graph passed"
            x = self.x_cache
        ...
        # 3) Dynamic prototypes (with gradient)
        assert self.S_raw is not None, "S_raw not set, please call update_event_U() first"
